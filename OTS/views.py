...
def candidateRegistration(request):
    if request.method=='POST':
        username=request.POST['username'] # name attribute of the form
...
def calculateTestResult(request):
    ...
    total_wrong = 0
    qid_list=[]
    for k in request.POST: # To retrive hidden input that is id of questions
        if k.startswith('qno'):
